...
public class BlockchainMember {
  ...
  private ConcurrentMap<Integer, ConsensusInstance> consensusInstances =
  ...
  private final int transactions_threshold =
      Dotenv.load().get("TRANSACTIONS_THRESHOLD") != null
  ...
  public BlockchainMember(int memberId, int memberPort, int leaderId, int f) {
    ...
    if (configFilePath == null || keysFolderPath == null) {
      Logger.log(
          LogLevel.ERROR,
          "Environment variables CONFIG_FILE_PATH or KEYS_FOLDER_PATH are not set.");
      return;
    }
    ...
    try {
      this.blockchain =
          new Blockchain(this.memberId, Config.getClientPublicKeys());
    } catch (IOException e) {
      Logger.log(LogLevel.ERROR,
                 "Failed to initialize Blockchain: " + e.getMessage());
    }

    PerfectLink pl;
    try {
      pl = new PerfectLink(memberId, memberPort, Config.processAddresses,
                           Config.getPrivateKey(memberId), Config.publicKeys);
    } catch (Exception e) {
      Logger.log(LogLevel.ERROR,
                 "Failed to create PerfectLink: " + e.getMessage());
      return;
    }
    this.perfectLink = pl;
  ...
  public static void main(String[] args) throws Exception {
    ...
    BlockchainMember blockchainMember =
        new BlockchainMember(memberId, memberPort, leaderId, 1);
    Logger.log(LogLevel.INFO, "BlockchainMember " + memberId +
                                  " started on port " + memberPort);
  }
  ...
  private void startMessageHandler() {
    while (true) {
      try {
        Message msg =
            perfectLink.deliver(); // waits for new elements to be added to the
        ...
        new Thread(() -> {
          ...
          ConsensusInstance consensusInstance =
              consensusInstances.get(msg.getClientId());
          ...
          if (msg.getType() == Message.Type.CLIENT_REQUEST) {
            ...
            default:
              ...
              Logger.log(LogLevel.ERROR,
                         "Unknown request type: " + msg.getRequestType());
              break;
            }
            ...
            if (consensusInstance == null) {
              ...
              consensusInstance = new ConsensusInstance(
                  memberId, leaderId, allProcessIds, perfectLink,
          ...
          if (decidedBlock != null) {
            try {
              ...
              Logger.log(LogLevel.WARNING,
                         "Blockchain updated: " +
                             this.blockchain.getHashesChain());
              ...
              InetSocketAddress clientAddr =
                  Config.processAddresses.get(msg.getClientId());
  ...
  public ConsensusInstanceAndBlock verifyTransactionAndAddOrProposeBlock(
      Message msg, ConsensusInstance consensusInstance, Block decidedBlock) {
    ...
    if (pendingTransactions.size() >= transactions_threshold) {
      ...
      try {
        ...
        Logger.log(LogLevel.ERROR,
                   "Failed to generate block hash: " + e.getMessage());
        return new ConsensusInstanceAndBlock(consensusInstance, decidedBlock);
      }
      ...
      if (consensusInstance != null) {
        ...
        consensusInstance = new ConsensusInstance(
            memberId, leaderId, allProcessIds, perfectLink, epochNumber++, f,
            block, msg.getClientId());
        consensusInstances.put(msg.getClientId(), consensusInstance);
      }

      if (memberId == leaderId) {
        consensusInstance.setBlockchainMostRecentWrite(
            new TimestampValuePair(0, block));
  ...
  public static class ConsensusInstanceAndBlock {
    ...
    public ConsensusInstanceAndBlock(ConsensusInstance consensusInstance,
                                     Block decidedBlock) {
  ...
  public ArrayList<Block> getBlockchain() { return this.blockchain.getChain(); }
  ...
  public Transaction processDepCoinTransfer(Transaction t, String senderAddress,
  ...
  public Transaction processBlockOperation(Transaction t,
    ...
    case TRANSFER_IST_COIN:
        ...
        boolean validTransaction = smartAccount.transfer(
            senderAddress, targetAddress, amount);
        if (validTransaction) {
            t.setStatus(Transaction.TransactionStatus.CONFIRMED);
            Logger.log(LogLevel.INFO, "Transaction processed: " + t);
        } else {
  ...
  public void processReadOperation(Message msg) {
    ...
    try {
      ...
      case GET_DEPCOIN_BALANCE:
        ...
        break;
      case GET_ISTCOIN_BALANCE:
        BigInteger balanceIST =
            smartAccount.balanceOf(senderAddress, targetAddress);
        replyValue = balanceIST.toString();
        break;

      case IS_BLACKLISTED:
        boolean isBlacklisted =
            smartAccount.isBlacklisted(senderAddress, targetAddress);
      ...
      case ALLOWANCE:
        ...
        BigInteger allowance =
            smartAccount.allowance(senderAddress, targetAddress, spender);
        replyValue = allowance.toString();
        break;

      default:
        Logger.log(LogLevel.ERROR, "Unknown read operation request type: " +
                                       msg.getRequestType());
        return;
      }
    } catch (NoSuchAlgorithmException e) {
      Logger.log(LogLevel.ERROR,
                 "Failed to process read operation: " + e.getMessage());
      return;
    }
  ...
  public boolean checkTransactionSignature(Transaction transaction,
